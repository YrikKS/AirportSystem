...
class LargeTechnicalInspectionFragment : Fragment() {
    ...
    private lateinit var model: LargeTechnicalInspectionViewModel
    private lateinit var binding: FragmentPlaneBinding
    private lateinit var adapter: LargeTechnicalInspectionAdapter

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        requireContext().appComponent.inject(this)
        model = ViewModelProvider(this, modelFactory)[LargeTechnicalInspectionViewModel::class.java]
    }
    ...
    private fun setupUi() {
        adapter = LargeTechnicalInspectionAdapter(emptyList()) { createDialog(it) }
        ...
        binding.title.text = "Large technical inspection"
    ...
    private fun generateInitTextField(inspection: LargeTechnicalInspection): List<Pair<String, (String) -> Unit>> {
        return listOf(
            "Full date (yyyy-mm-dd hh:mm:ss)" to { inspection.date = getTimestampFrom(it) },
            "Result inspection" to { inspection.resultInspection = it },
        )
    }

    private fun generateInitPickField(inspection: LargeTechnicalInspection): List<Triple<String, (DbEntity) -> Unit, Pair<suspend () -> List<DbEntity>, (DbEntity) -> String>>> {
        return listOf(
            Triple(
                "Plane",
                { inspection.idPlane = it.customGetId() },
                suspend { model.getPlanes() } to
                        { (it as Plane).modelPlane!!.nameModel + " " + it.id }),
            Triple(
                "Brigade",
                { inspection.idInspectionTeam = it.customGetId() },
    ...
    private fun generateUpdateTextField(
        inspection: LargeTechnicalInspection,
        newInspection: LargeTechnicalInspection
    ): List<Pair<Pair<String, String>, (String) -> Unit>> {
        return listOf(
            ("Full date (yyyy-mm-dd hh:mm:ss)" to inspection.date.toString()) to {
                newInspection.date = getTimestampFrom(it)
            },
            ("Report" to inspection.resultInspection) to { newInspection.resultInspection = it },
        )
    }
    ...
    private fun generateUpdatePickField(
        ...
        return listOf(
            Triple(
                "Plane" to inspection.planeEntity?.modelPlane?.nameModel + " " + inspection.planeEntity?.id,
                { newInspection.idPlane = it.customGetId() },
                suspend { model.getPlanes() } to
                        { (it as Plane).modelPlane!!.nameModel + " " + it.id }),
            Triple(
                "Brigade" to (inspection.brigade?.nameBrigade ?: "Unknown"),
                { newInspection.idInspectionTeam = it.customGetId() },
                suspend { model.getBrigades() } to
                        { (it as Brigade).nameBrigade }),
    ...
    private fun createDialog(report: LargeTechnicalInspection) {
    ...
    private fun openBottomDialogInsert() {
        BottomSheetDialog(requireContext()).apply {
            val dialog = BottomDialogBinding.inflate(LayoutInflater.from(requireContext()))
            setContentView(dialog.root)
            val newReport = LargeTechnicalInspection()
    ...
    companion object {
        fun newInstance() = LargeTechnicalInspectionFragment()
    }
}