...
class AirportFragment : Fragment() {
    ...
    private lateinit var model: AirportViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        model = ViewModelProvider(this, modelFactory)[AirportViewModel::class.java]
    }
    ...
    private fun setupUi() {
        adapter = AirportAdapter(emptyList()) { createDialog(it) }
    ...
    private fun generateInitTextField(airport: Airport): List<Pair<String, (String) -> Unit>> {
        return listOf(
            "City" to { airport.city = it },
            "Airport" to { airport.airportName = it },
        )
    }

    private fun generateInitPickField(airport: Airport): List<Triple<String, (DbEntity) -> Unit, Pair<suspend () -> List<DbEntity>, (DbEntity) -> String>>> {
        return listOf()
    }
    ...
    private fun generateUpdateTextField(
        ...
        return listOf(
            ("City" to airport.city) to { newAirport.city = it },
            ("Airport" to airport.airportName) to { newAirport.airportName = it },
        )
    ...
    private fun openBottomDialogInsert() {
        BottomSheetDialog(requireContext()).apply {
            ...
            dialog.bottomLayout.addInsertTextField(
                generateInitTextField(newAirport),
                requireContext()
            )
            dialog.bottomLayout.addInsertPickField(
                generateInitPickField(newAirport),
                lifecycleScope,
                requireContext()
            )
            dialog.bottomLayout.addInsertButton(context) {
                model.insert(newAirport)
                dismiss()
            }
        }.show()
    }
