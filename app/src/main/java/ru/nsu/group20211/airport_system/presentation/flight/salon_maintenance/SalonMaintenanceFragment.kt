...
class SalonMaintenanceFragment : Fragment() {
    @Inject
    lateinit var modelFactory: ViewModelProvider.Factory

    private lateinit var model: SalonMaintenanceViewModel
    private lateinit var binding: FragmentPlaneBinding
    private lateinit var adapter: SalonMaintenanceAdapter

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        requireContext().appComponent.inject(this)
        model = ViewModelProvider(this, modelFactory)[SalonMaintenanceViewModel::class.java]
    }
    ...
    private fun setupUi() {
        adapter = SalonMaintenanceAdapter(emptyList()) { createDialog(it) }
        ...
        binding.title.text = "Salon maintenances"
    ...
    private fun generateInitTextField(entity: SalonMaintenance): List<Pair<String, (String) -> Unit>> {
        return listOf(
            "Date (yyyy-mm-dd hh:mm:ss)" to { entity.date = getTimestampFrom(it) },
            "Report" to { entity.report = it },
        )
    }

    private fun generateInitPickField(entity: SalonMaintenance): List<Triple<String, (DbEntity) -> Unit, Pair<suspend () -> List<DbEntity>, (DbEntity) -> String>>> {
        return listOf(
            Triple(
                "Flight",
            ...
            Triple(
                "Brigade",
                { entity.idCleaningTeamSalon = it.customGetId() },
    ...
    private fun generateUpdateTextField(
        entity: SalonMaintenance,
        newEntity: SalonMaintenance
        ...
        return listOf(
            ...
            ("Report" to entity.report) to { newEntity.report = it },
        )
    }
    ...
    private fun generateUpdatePickField(
        ...
        return listOf(
            ...
            Triple(
                ...
                { newEntity.idCleaningTeamSalon = it.customGetId() },
                suspend { model.getBrigades() } to
                        { (it as Brigade).nameBrigade }),
        )
    }
    ...
    private fun createDialog(entity: SalonMaintenance) {
    ...
    private fun openBottomDialogUpdate(entity: SalonMaintenance) {
    ...
    private fun openBottomDialogInsert() {
        BottomSheetDialog(requireContext()).apply {
            val dialog = BottomDialogBinding.inflate(LayoutInflater.from(requireContext()))
            setContentView(dialog.root)
            val newEntity = SalonMaintenance()
    ...
    companion object {
        fun newInstance() = SalonMaintenanceFragment()
    }
}