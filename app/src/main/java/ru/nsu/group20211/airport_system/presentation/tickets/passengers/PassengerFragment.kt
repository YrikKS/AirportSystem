...
class PassengerFragment : Fragment() {
    ...
    private lateinit var model: PassengerViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        model = ViewModelProvider(this, modelFactory)[PassengerViewModel::class.java]
    ...
    private fun setupUi() {
        adapter = PassengerAdapter(emptyList()) { createDialog(it) }
        ...
        binding.title.setText("Passenger")
        model.stateProvider
            .onEach {
    ...
    private fun createDialog(human: Passenger) {
    ...
    private fun generateSidePickParamNoInternet(filter: DbFilter): List<Triple<String, List<String>, (String?) -> Unit>> {
        return listOf(
            Triple(
                ...
                if (it != null) {
                    filter.queryMap[3] = """ "passengers"."sex" = '${it.first()}' """
                } else {
    ...
    private fun openBottomDialogUpdate(human: Passenger) {
                        ...
                        dialog.bottomLayout.forEachIndexed { index, view ->
                            mapElement.forEach { key, view ->
                                when (key) {
                                    ...
                                    "Pyrtonymic" -> newHuman.patronymic = if (view.text == null) {
                                        null
                                    } else {
                                        view.text.toString()
                                    }
        ...
        BottomSheetDialog(requireContext()).apply {
            ...
            dialog.bottomLayout.addView(
                DialogInflatorBinding.inflate(
                    ...
                    it.bottomButton.setOnClickListener {
                        val newHuman = Passenger()
                        dialog.bottomLayout.forEachIndexed { index, view ->
                            mapElement.forEach { key, view ->
                                when (key) {
                                    "Name" -> newHuman.name = view.text.toString()
                                    "Surname" -> newHuman.surname = view.text.toString()
                                    "Patronymic" -> newHuman.patronymic = if (view.text == null) {
                                        null
                                    } else {
                                        view.text.toString()
                                    }
